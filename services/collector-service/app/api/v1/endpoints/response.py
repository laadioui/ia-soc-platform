...
async def block_ip(data: BlockIPRequest, db: AsyncSession = Depends(get_db)):
    action = await _create_response_action(
        db, data.incident_id, "block_ip", data.ip_address, {"reason": data.reason}
    )
    return action
...
async def disable_user(data: DisableUserRequest, db: AsyncSession = Depends(get_db)):
    action = await _create_response_action(
        db, data.incident_id, "disable_user", data.username, {"reason": data.reason}
    )
    return action
...
async def kill_session(data: KillSessionRequest, db: AsyncSession = Depends(get_db)):
    action = await _create_response_action(
        db, data.incident_id, "kill_session", data.session_id
    )
    return action
...
async def reset_password(data: ResetPasswordRequest, db: AsyncSession = Depends(get_db)):
    action = await _create_response_action(
        db, data.incident_id, "reset_password", data.username
    )
    return action
...
async def create_firewall_rule(
    ...
    action = await _create_response_action(
        db, incident_id, "create_firewall_rule", rule_description
    )
    return action
